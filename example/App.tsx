...
export default function App() {
  ...
  const saveFile = async (name: string, type: string, data: string, encoding?: "base64" | "utf8") => {
    setError(null);
    setResult(null);
    try {
      ...
      const result = await Downloads.saveFile({ name, type, data, encoding });
      if (result.cancelled) {
        setError("Download cancelled");
      } else {
        setResult({ uri: result.uri, type });
      }
    } catch (err: any) {
  ...
  const saveImageFile = async () => {
    ...
    await saveFile(name, type, base64Data, "base64");
  };
  ...
  const saveLargeFile = async () => {
    ...
    const data = baseText.repeat(repeats);
    await saveFile(name, type, data);
  };
  ...
  const openFile = async () => {
    if (!result) return;
    try {
      await Downloads.openFile({ uri: result.uri, type: result.type });
    } catch (err: any) {
      setError(err.message || "An error occurred");
    }
  };
